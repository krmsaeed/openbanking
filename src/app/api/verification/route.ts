...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!signature || !video || !type) {
            return NextResponse.json({ error: 'اطلاعات ناقص ارسال شده است' }, { status: 400 });
        }

        const referenceId =
            `VER_${window !== undefined && Date.now()}_${Math.random().toString(36).substring(2, 8)}`.toUpperCase();
...
export async function GET(request: NextRequest) {
    ...
    if (!referenceId) {
        return NextResponse.json({ error: 'Reference ID الزامی است' }, { status: 400 });
    }
    try {
        const status = {
            referenceId,
        ...
        return NextResponse.json({ error: 'خطا در بازیابی اطلاعات' }, { status: 500 });
    }
}
