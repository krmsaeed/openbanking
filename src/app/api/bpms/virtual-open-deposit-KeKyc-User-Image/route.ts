...
import { virtualOpenDepositKeKycUserImage } from '@/services/bpms';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { serviceName, processId, formName, body } = message;

        if (serviceName !== 'virtual-open-deposit' || formName !== 'GovahInquiry' || !processId || !body.code) {
            return NextResponse.json({ error: 'Invalid messageDTO' }, { status: 400 });
        }

        const resp = await virtualOpenDepositKeKycUserImage(body, processId, files);
        ...
        console.error('Error in virtual-open-deposit-KeKyc-User-Image:', error);
