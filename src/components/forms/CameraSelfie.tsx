            ...
            try {
                ...
                try {
                    ...
                    if (targetSkin !== null) {
                        ...
                        // Allow slightly farther when using a calibrated target
                        centerBoxClose = percentRel >= 90; // was 95
                    } else {
                        const minSkin = 0.08;
                        const maxSkin = 0.35;
                        ...
                        // Be a bit more forgiving on closeness so user can stand slightly farther
                        centerBoxClose = percent >= 75; // was 85
                    }
            ...
            <Box className="relative bg-black rounded-full overflow-hidden w-70 h-70 mx-auto">
                ...
                {!capturedPhoto && stream && (
                    <div className="absolute inset-0 z-30 pointer-events-none flex items-center justify-center">
                        <div className={`w-52 h-28 rounded-lg border-2 transition-colors duration-200 ${eyesCentered ? 'border-green-400' : 'border-yellow-400'} opacity-80`}></div>
                        <div className="absolute top-3 text-center text-xs text-white w-full px-4">
                            <div>{eyesCentered ? 'چشم‌ها در مرکز قاب هستند' : `لطفاً چشم‌ها را به مرکز نزدیک کنید (${eyeOffsetPercent}% جابجایی)`}</div>
                        </div>
                    </div>
                )}
                            ...
                            faceTooFar ? 'لطفاً نزدیک‌تر بیایید' :
                                obstructionRatio >= MAX_OBSTRUCTION ? 'لطفاً دست یا اشیاء را از جلوی صورت بردارید' :
                                    eyeFeatureRatio < MIN_EYE_RATIO ? 'لطفاً مطمئن شوید چشم‌ها و ابروها به وضوح دیده می‌شوند' :
                                        !eyesCentered ? 'لطفاً چشم‌ها را به مرکز قاب بیاورید' :
            ...
            {!capturedPhoto && stream && !isLoading && (
                <Box className="flex flex-col items-center space-y-2">
                    <button
                        ...
                        disabled={closenessPercent < 95 || obstructionRatio >= 0.15 || !eyesCentered || !faceDetected}
                        className={`w-20 h-20 rounded-full border-4 transition-all duration-300 ${closenessPercent >= 95 && obstructionRatio < 0.15 && eyesCentered
                            ? 'bg-green-500 border-green-600 hover:bg-green-600 shadow-lg hover:shadow-xl active:scale-95'
                            : 'bg-gray-300  cursor-not-allowed opacity-50'
                            }`}
                        title={closenessPercent >= 95 && obstructionRatio < 0.15 && eyesCentered ? 'عکس بگیرید' : 'ابتدا نزدیکی را به حداقل 95٪ برسانید، چشم‌ها را در مرکز قرار دهید و مطمئن شوید که هیچ مانعی وجود ندارد'}
                    >
                        <Box className={`w-12 h-12 rounded-full mx-auto transition-colors ${closenessPercent >= 95 && obstructionRatio < 0.15 ? 'bg-white' : 'bg-gray-500'
                            }`} />
                    </button>
                </Box>
            )}
