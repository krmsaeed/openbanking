...
export default function Sidebar({ step = 1, onSelect }: SidebarProps) {
    return (
        <nav className="bg-white rounded-lg shadow-lg p-4 w-full max-w-md mx-auto h-full" aria-label="مراحل ثبت‌نام">
                            ...
                            <Box className="flex flex-col items-center md:flex-row md:items-center w-full">
                                <Box className="relative flex items-center justify-center">
                                    <span className={`
                                        ...
                                        ${completed ? 'bg-primary border-primary text-white scale-100' : ''}
                                        ${current ? 'bg-white border-primary text-primary scale-105' : 'bg-white border-gray-300 text-gray-400'}`
                                    }>
                                        {completed ? <CheckCircleIcon className="w-6 h-6 mx-auto" /> : <Icon className="w-5 h-5 mx-auto" />}
                                    </span>
