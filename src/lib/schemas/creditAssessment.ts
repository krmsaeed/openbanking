...
export const financialInfoSchema = z.object({
    monthlyIncome: z.string().min(1, "درآمد ماهانه ضروری است").refine((val) => !isNaN(Number(val)) && Number(val) > 0, "درآمد باید عدد مثبت باشد"),
    otherIncome: z.string().optional(),
    monthlyExpenses: z.string().optional(),
    workExperience: z.string().min(1, "سابقه کاری ضروری است").refine((val) => !isNaN(Number(val)) && Number(val) >= 0, "سابقه کاری باید عدد باشد"),
    jobTitle: z.string().optional(),
    companyName: z.string().optional(),
    workAddress: z.string().optional(),
    requestedAmount: z.string().min(1, "مبلغ درخواستی ضروری است").refine((val) => !isNaN(Number(val)) && Number(val) > 0, "مبلغ باید عدد مثبت باشد"),
